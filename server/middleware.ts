...
export async function requireAuth(req: Request, res: Response, next: NextFunction) {
  logger.debug('[DEBUG] requireAuth middleware called for:', req.method, req.path);
  try {
    ...
    logger.debug('[DEBUG] Authorization header:', authHeader);
  ...
  try {
    ...
    if (user && user.role === 'admin') {
      ...
      logAccess(
        req,
        req.customUser.id,
        req.customUser.email || 'unknown',
        `Admin accessed case ${caseId}`
      );
      return next();
    }
...
export async function requireAdmin(
  ...
  try {
    ...
    if (!req.user || !req.user.id) {
      logger.debug('[DEBUG] Admin middleware: No user or user.id found');
      return res.status(401).json({
        error: 'Authentication required',
    ...
    logger.debug('[DEBUG] Admin middleware: Checking user', userId);

    // Get user from storage to check role
    const user = await storage.getUser(userId);
    logger.debug('[DEBUG] Admin middleware: User from DB', user?.email, user?.role);

    if (!user || user.role !== 'admin') {
      // Log unauthorized admin access attempt
      logger.debug(
        `[SECURITY] Unauthorized admin access attempt by user: ${user?.email || 'unknown'} (${userId})`
      );
      return res.status(403).json({
        error: 'Access denied',
...
function logAccess(req: Request, userId: string, userEmail: string, additionalInfo?: string) {
  ...
  logger.debug(
    `[AUDIT] ${timestamp} | User: ${userEmail} (${userId}) | ${method} ${path} | IP: ${ip} | ${additionalInfo || 'API access'}`
  );
...
export function logAccessAttempt(req: Request, res: Response, next: NextFunction) {
  ...
  logger.debug(`[ACCESS_ATTEMPT] ${timestamp} | ${method} ${originalUrl} | IP: ${ip}`);
