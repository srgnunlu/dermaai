...
import { Microscope, Activity, History, Settings, User as UserIcon, LogOut, Shield, Bell } from "lucide-react";
                ...
                {navigation.map((item) => {
                  ...
                  return (
                    <DropdownMenuItem key={item.name} asChild>
                      <Link href={item.href}>
                        <a className="flex w-full items-center" data-testid={`mobile-link-${item.name.toLowerCase().replace(" ", "-")}`}>
      ...
      <Dialog open={alertsOpen} onOpenChange={setAlertsOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Track urgent dermatology cases and recent AI analyses. Adjust delivery preferences from Settings → Notifications.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-6">
            <section>
              <h3 className="text-xs font-semibold uppercase tracking-wide text-red-300">Urgent Cases</h3>
              <div className="mt-3 rounded-lg border border-border bg-card/60">
                {casesLoading ? (
                  <p className="px-4 py-6 text-sm text-muted-foreground">Checking for urgent alerts…</p>
                ) : urgentCases.length > 0 ? (
                  <ScrollArea className="max-h-48">
                    <ul className="divide-y divide-border">
                      {urgentCases.map((caseItem) => {
                        const urgentDiagnosis = caseItem.finalDiagnoses?.find((diagnosis) => diagnosis.isUrgent);
                        return (
                  ...
                  <p className="px-4 py-6 text-sm text-muted-foreground">No urgent cases currently flagged.</p>
                )}
              </div>
            </section>

            <section>
              <h3 className="text-xs font-semibold uppercase tracking-wide text-blue-200">Recent Analyses</h3>
              <div className="mt-3 rounded-lg border border-border bg-card/60">
                {casesLoading ? (
                  <p className="px-4 py-6 text-sm text-muted-foreground">Loading recent activity…</p>
                      ...
                      {recentCases.map((caseItem) => (
                        <li key={caseItem.id} className="px-4 py-3 text-sm">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs uppercase text-muted-foreground">{caseItem.status}</span>
                          </div>
                          <p className="text-muted-foreground">
                  ...
                  <p className="px-4 py-6 text-sm text-muted-foreground">No analyses yet. Upload a case to get started.</p>
                )}
              </div>
            </section>
          </div>
