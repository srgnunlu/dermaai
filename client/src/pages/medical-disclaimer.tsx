    ...
    <div className="min-h-screen bg-background flex flex-col">
      <main className="flex-1">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <h1 className="mt-4 text-4xl font-bold text-foreground">Clinical Oversight Remains Essential</h1>
          <p className="mt-4 text-lg text-muted-foreground leading-relaxed">
            DermaAI augments dermatology workflows with AI-driven insights. It does not replace professional judgement, diagnostic testing, or patient consultation. Review the statements below with your clinical governance team before deploying the platform.
          </p>

          <div className="mt-10 space-y-4">
            {statements.map((entry) => (
              <div key={entry} className="rounded-xl border border-border bg-card/80 p-5">
                <p className="text-sm md:text-base text-muted-foreground leading-relaxed">{entry}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-8 rounded-xl border border-yellow-400/40 bg-yellow-500/5 p-5 text-sm text-yellow-200">
            ...
            <p className="mt-2">
              Assign a safety officer responsible for reviewing AI-assisted cases, documenting overrides, and coordinating feedback with the DermaAI medical science team.
            </p>
          </div>
        </div>
      </main>
