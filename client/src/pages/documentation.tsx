      ...
      <main className="flex-1">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 text-4xl font-bold text-foreground">Implementing DermaAI in Your Clinic</h1>
            <p className="mt-4 text-lg text-muted-foreground leading-relaxed">
              Access deployment guides, workflow diagrams, and API references to configure DermaAI for your dermatology service. These resources assume a technical lead collaborating with clinical champions.
            </p>
          </div>

          <div className="mt-10 grid gap-6 md:grid-cols-2">
            {modules.map(({ icon: Icon, title, description }) => (
              <div key={title} className="rounded-2xl border border-border bg-card/80 p-6 shadow-sm">
          ...
          <div className="mt-12 rounded-2xl border border-border bg-card/80 p-6">
            ...
            <ol className="mt-4 list-decimal space-y-2 pl-6 text-sm text-muted-foreground">
              <li>Review the onboarding guide with your clinical governance team and appoint a DermaAI administrator.</li>
              <li>Configure secure S3 or Cloudinary storage (if required) and verify outbound network policies to DermaAI endpoints.</li>
              <li>Execute a pilot with de-identified historic cases to familiarise clinicians with consensus reporting and confidence scores.</li>
              <li>Document sign-off and publish the use-case to your staff intranet or knowledge base.</li>
            </ol>
          </div>
        </div>
      </main>
